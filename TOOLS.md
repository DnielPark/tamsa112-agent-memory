...
### AWS 서버 인프라 (AWS Lightsail)
- **설명**: 소설 편집기가 호스팅된 AWS Lightsail 서버 정보. IP, 도메인, 인증, SSL 등 인프라 정보.
- **사용 시기**: 서버 접속, 인증 정보 확인, 인프라 관련 질문 시
- **스킬 위치**: `./skills/aws-server-info.md`

### 토목 단가 관리 (Civil Cost Manager)
- **설명**: 토목 단가산출서 관리. 단가 체계 계층 구조, 입력 규칙, 프로젝트 정보. 단가 작업 시 먼저 읽고 작업.
- **사용 시기**: 단가 추가/수정, 서버 관련 작업 시
- **스킬 위치**: `./skills/unit-cost-structure.md`
- **GitHub 레포**: `DnielPark/civil-cost-manager`

### 에이전트 모델 관리
- **설명**: OpenClaw 에이전트(writer/main)의 모델 설정 및 API 키 관리. 미지원 모델(DeepSeek 등) 수동 추가 방법 포함.
- **사용 시기**: API 키 변경, 모델 추가, 에이전트 설정 확인 필요 시
- **스킬 위치**: `./skills/agent-model-management.md`
...
### 서버 배포 워크플로우
- **설명**: 서버 파일 수정 및 배포 절차. GitHub 경유 방식, 담당자 판단 기준, Claude 컨펌 필수 규칙.
- **사용 시기**: 서버 파일 수정/배포 요청 시 반드시 먼저 읽을 것
- **스킬 위치**: `./skills/server-deploy-workflow.md`
...
```bash
cd ~/.openclaw/workspace/skills
./civil-cost-manager-pull.sh    # 1. 작업 시작 전
# 단가 파일 수정                # 2. civil-cost-manager/ 안에서
./civil-cost-manager-push.sh    # 3. 수정 완료 후
```
...
├── MEMORY.md — 장기 기억
└── skills/
    ├── agent-model-management.md — 에이전트 모델 관리
    ├── aws-server-info.md — AWS 서버 인프라
    ├── server-deploy-workflow.md — 서버 배포 워크플로우
    ├── github-workflow.md — GitHub 배포 워크플로우
    ├── web-search.md — 웹 검색 API
    ├── novel-brain.md — 소설 보조작가
    ├── unit-cost-structure.md — 토목 단가 구조
    ...
    ├── civil-cost-manager-pull.sh — GitHub → 맥북 (단가 파일)
    ├── civil-cost-manager-push.sh — 맥북 → GitHub (단가 파일)
    ├── civil-cost-manager-app-on.sh — 단가 앱 시작
    ├── civil-cost-manager-app-off.sh — 단가 앱 종료
